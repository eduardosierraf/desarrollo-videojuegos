# Desarrollo de Videojuegos

**Programa:** Ingeniería de Sistemas — Universidad de La Guajira
**Docente:** Eduardo Luis Sierra Fragozo ([esierra@uniguajira.edu.co](mailto:esierra@uniguajira.edu.co))

Material de la asignatura Desarrollo de Videojuegos (Unity), independiente de periodo académico.

## Contenido del repositorio

- `index.html` — presentación de la asignatura (portada, panorama de la industria, proyectos del curso, stack, temario, evaluación, asistencia y requisitos técnicos).
- `assets/` — recursos gráficos de la presentación (vacía por ahora).

## Cómo abrir la presentación

Abre `index.html` directamente en el navegador. No requiere build ni dependencias — es HTML/CSS/JS plano.

Navegación por teclado: flechas / `Page Up` / `Page Down` / `Space` para avanzar, `Home` / `End` para ir al inicio o al final.
